...
public:
    PerspectiveCamera(const PropertyList &propList) {
        ...
        m_focalDistance = propList.getFloat("focalDistance", 0.f);
    ...
    Color3f sampleRay(Ray3f &ray,
        ...
        Point3f nearP = m_sampleToCamera * Point3f(samplePosition.x() * m_invOutputSize.x(),
                                                   samplePosition.y() * m_invOutputSize.y(),
                                                   0.f);
        
        float invZ = 1.f / nearP.normalized().z();
        
        // Compute origin and direction (in local coordinates)
        ray.o = Point3f(0.f);
        ...
        if (m_lensRadius > 0.f) {
            ...
            Point3f pf = ray.operator()(m_focalDistance / ray.d.z());
            
            // Update ray
            ray.o = Point3f(pLens.x(), pLens.y(), 0.f);
            ray.d = (pf - ray.o).normalized();
        }
